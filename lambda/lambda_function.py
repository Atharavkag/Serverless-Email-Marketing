...
def lambda_handler(event, context):
    ...
    bucket_name = 'email-marketing01' # Replace with your bucket name

    try:
        # Retrieve the CSV file from S3
        csv_file = s3_client.get_object(Bucket=bucket_name, Key='contact.csv')
        lines = csv_file['Body'].read().decode('utf-8').splitlines()
        
        # Retrieve the HTML email template from S3
        email_template = s3_client.get_object(Bucket=bucket_name, Key='template1.html')
        ...
        for contact in contacts:
            ...
            response = ses_client.send_email(
                Source='atharavkagde7@gmail.com',  # Replace with your verified "From" address
                Destination={'ToAddresses': [contact['Email']]},
                Message={
                    'Subject': {'Data': 'Your Weekly TechWave Mail!', 'Charset': 'UTF-8'},
